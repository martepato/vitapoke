...
| `scripts/` | `common.sh` (sourced by every script: it resolves which toolchain is in play -- the pinned download, or your own install when `VITASDK` is set -- and exports that decision so a child step cannot re-derive it differently), `toolchain.sh` (pinned VitaSDK), `deps.sh` (vitaGL, vitaShaRK, math-neon, the SDL declarations libntr wants; asks before writing into an install it did not create, and `VITAPOKE_SKIP_DEPS=1` leaves the libraries already there alone), `fetch.sh` (pinned upstream sources), `stage.sh` (lays out the build tree), `game.sh` (the build), `prereqs.sh` (checks host tools and offers to install what is missing; `VITAPOKE_ASSUME_YES=1` skips the prompt), `extract_assets.py` (unpacks a ROM's file system for the VPK), `make_save.py` (a blank save for a desktop test; the port makes its own on the console), `png_embed.py`, `sdk_archive.py`. |
